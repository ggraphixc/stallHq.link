 ...
 * Uses the platform-configured LLM (OpenRouter / OpenAI / custom)
...
- Output valid JSON only: { "title": "...", "body": "..." }`;
    ...
    try {
      ...
      // If JSON parsing fails, use the raw content split by lines
    ...
    if (!title) title = "💡 " + (type === "promo" ? "New offer available" : "Quick tip for your store");
    if (!msgBody) msgBody = prompt.slice(0, 120);
