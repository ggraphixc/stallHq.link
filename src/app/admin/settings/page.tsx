...
const DEFAULT_SETTINGS: Record<string, any> = {
  ...
  ai_enabled: false, ai_provider: "openrouter", ai_model: "gemini-3.5-flash", ai_api_key: "", ai_base_url: "", ai_assistant_enabled: false,
          ...
          <div>
            ...
            {settings.ai_provider === "google" && (
              <p style={{ fontSize: "0.625rem", color: "var(--text-muted)", marginTop: "0.25rem" }}>
                ...
                {" "}. Free tier available. Use gemini-3.5-flash (cheap) or gemini-3.5-flash-lite (smarter, 6x cost if you exceed free tier).
              </p>
            )}
          </div>
          <div>
            ...
            {settings.ai_provider === "google" ? (
              <select className="ambient-input" style={{ width: "100%", padding: "0.625rem 0.875rem", fontSize: "0.8125rem", borderRadius: "0.5rem", background: "var(--bg-primary)", boxSizing: "border-box" }} value={settings.ai_model || "gemini-3.5-flash"} onChange={(e) => updateSetting("ai_model", e.target.value)}>
                <option value="gemini-3.5-flash">gemini-3.5-flash (cheap, 250 req/day free)</option>
                <option value="gemini-3.5-flash-lite">gemini-3.5-flash-lite (smarter, 6x cost over free tier)</option>
              </select>
            ) : (
