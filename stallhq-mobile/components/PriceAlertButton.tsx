...
/**
 * Price drop alert toggle button.
...
export function PriceAlertButton({ productId }: Props) {
  ...
  async function checkAlert(id: string) {
    try {
      const res = await fetch(
        `${WEB_API_URL}/api/alerts/price?product_id=${productId}&device_id=${id}`
      );
      if (res.ok) {
  ...
  async function toggle() {
    ...
    try {
      if (subscribed) {
        ...
        await fetch(`${WEB_API_URL}/api/alerts/price`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ product_id: productId, device_id: deviceId, target_price: 0 }),
        });
        setSubscribed(true);
      }
    } catch {} finally {
  ...
  return (
    ...
    >
      {subscribed ? (
        <BellOff size={16} color={Colors.purple} />
      ) : (
        <Bell size={16} color={Colors.textSecondary} />
      )}
      <Text style={[styles.text, subscribed && styles.textActive]}>
        {subscribed ? "Alert Set" : "Price Alert"}
      </Text>
    </Pressable>
  );
}
