...
export default function CustomerChatThreadScreen() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const fromMeta = rows.reduce<ReactionSummary>(
          (acc, m) => mergeReactionSummaries(acc, { [m.id]: [] }),
          {}
        );
        void fromMeta;
  ...
  const replyFor = (id: string): { name?: string; preview: string } => {
    ...
    if (target) {
      return {
        name: target.sender_id === userId ? "You" : "Vendor",
  ...
  const sendMessage = async (imageUri?: string) => {
    ...
    const optimistic: DmMessage = {
      ...
      content: imageUri ? displayContent(imageUri) : content,
      read_at: null,
  ...
  const typingLabel =
    typingPeers.length > 0 ? "typing…" : undefined;

  const avatar = useMemo(() => {
    const [c1, c2] = avatarColors(conversationId || storeName);
    return (
      <LinearGradient colors={[c1, c2]} style={styles.headerAvatar}>
        <Text style={styles.headerAvatarText}>{storeName[0]?.toUpperCase()}</Text>
      </LinearGradient>
    );
  ...
  return (
    <SafeAreaView style={styles.container} edges={["top", "left", "right"]}>
      <ChatHeader
        onBack={() => router.back()}
        title={storeName}
        subtitle={<Text style={styles.headerSub}>Vendor</Text>}
        avatar={avatar}
        safeTop={insets.top}
        typing={!!typingLabel}
        typingLabel={typingLabel}
        replyName={reply ? (reply.sender_id === userId ? "You" : "Vendor") : null}
        ...
        >
          <FlatList
            ...
            renderItem={({ item: msg }) => {
              ...
              const img = isImageUrl(msg.content) || msg.content.startsWith("http") && /\.(png|jpe?g|webp|gif)/i.test(msg.content);
              return (
                <ChatBubble
                  content={msg.content}
                  mine={isMine}
