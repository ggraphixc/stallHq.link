...
alter table stores add column if not exists trial_ends_at timestamp with time zone default (timezone('utc'::text, now()) + interval '14 days');
...
comment on column stores.plan is 'Subscription plan: trial (14-day free), monthly (₦3,500/mo), quarterly (₦7,500/3mo), annual (₦12,000/6mo)';
comment on column stores.verified is 'Verified vendor badge (granted on quarterly+ plans)';
comment on column stores.trial_ends_at is 'When the 14-day free trial expires';
comment on column stores.subscription_expires_at is 'When the current paid subscription expires';

-- Update existing stores to have trial plan with 14-day expiry from now
